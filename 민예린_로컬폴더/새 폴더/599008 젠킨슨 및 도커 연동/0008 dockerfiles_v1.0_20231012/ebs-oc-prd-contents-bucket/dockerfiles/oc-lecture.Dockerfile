...
ENV JAVA_OPTS="-Xmx2g -Xms2g -javaagent:/whatap/whatap.agent-2.0_29.jar -Dwhatap.micro.enabled=true"
...
ENTRYPOINT exec java $JAVA_OPTS -Dspring.profiles.active=${SERVER_ENV} -Drun.arguments=-namespace=${NAMESPACE} -Dspring.config.location=${CONFIG_NAME} -jar /oc-lecture.jar